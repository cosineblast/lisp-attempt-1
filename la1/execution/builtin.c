...
int equals(Value *left_value, Value *right_value) {
    ...
    switch (left_value->type) {
        ...
        case LA1_VALUE_LIST:
            return list_equals(left_value->content.list, right_value->content.list);
...
Value *la1_builtin_emptyp(LA1_State *state, LinkedList *arguments, void *extra) {
    ...
    int result = argument->type == LA1_VALUE_LIST && argument->content.list == NULL;
